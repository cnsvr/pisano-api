...
RSpec.describe Api::V1::QuestionsController, type: :controller do
  ...
  describe 'GET #show' do
    ...
    describe 'POST #create' do
      ...
      it 'creates a question with options' do
        post :create, params: { title: 'Test Question', type: 1, survey_id: survey.id, options: [{ title: 'Test Option' }] }, as: :json
        expect(response).to have_http_status(:created)
        expect(parsed_json_body['title']).to eq('Test Question')
  ...
  describe 'PUT #update' do
    ...
    it 'updates a question with options' do
      put :update, params: { id: questions.first.id, title: 'Test Question', type: 1, options: [{ title: 'Test Option' }] }, as: :json
      expect(response).to have_http_status(:success)
      expect(parsed_json_body['title']).to eq('Test Question')
      expect(parsed_json_body['options'].first['title']).to eq('Test Option')
    end
